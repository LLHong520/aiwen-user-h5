<template>
  ...
  <layout class="doc-info-page">
    <!-- page_header -->
    <m-header slot="page_header" title='绑定手机' :menu_on='false'>
      ...
      <section class="post_topic_wrapper">
        <div class="form">
          ...
          <div class="inp_text_wrapper">
            ...
            <span>
              <input class='inp_text needsclick' @blur="validMobile()" name="mobile" v-model="mobile" placeholder="请输入11位手机号"
              />
            </span>
          </div>
          <div class="inp_text_wrapper valid_code">
            ...
            <span class="input">
              <input type="number" class='inp_text needsclick' name="mobile" min="6" max="6" v-model="valid_code" placeholder="请输入验证码"
              />
            </span>
        ...
        <div class="post_topic_action">
          <input type="button" @click="bind()" class='block_btn needsclick' :class="{'enable':enable}" name="" :value="binding? '绑定中...':'完成'"
          />
        </div>
      </section>
      <!-- /修改昵称 -->
...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      //     this.$toast.tip("手机号格式错误！");
      //     return;
      //   }
      //   let This = this;
      //   This.sending = true;
      //   userService.captchar({
