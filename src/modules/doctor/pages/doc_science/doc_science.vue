...
<layout class="doc-info-page" :docSwitch="true" :isShowNav="isShowNav">
  ...
  <div slot="page_header">
   <conHeader title='健康科普' :isWhite="true">
  ...
  <div slot="page_container" class="app_wrapper_inner">
    <div class="kepu-list">
      <ul class="article-list">
          <li v-for="(article,index) in kepuParam.list" :key="index">
      ...
      <loading-btn
        v-if="kepuParam.pager.totalPage>1"
        :loading="kepuParam.load_more_loading"
        :title="kepuParam.load_more_text" :isWhite="true"
        @loadFn="getKepuList">
      </loading-btn>
      <noResult v-if="!kepuParam.load_more_loading&&kepuParam.list.length==0" title="木有相关文章^_^"></noResult>
  </div>
<!-- /page_container -->
  <!-- page_footer -->
...
export default {
  ...
  methods:{
    ...
    getKepuList(){
      if(!this.kepuParam.hasMore||this.kepuParam.load_more_loading) return;
      this.kepuParam.load_more_loading=true;
      let This=this;
      let targetPageNum=this.kepuParam.pager.nowPage+1;
      sysService.getArticleByType({
        ...
        if(res.code==0&&res.data){
          This.kepuParam.list = This.kepuParam.list.concat(res.data);
          This.kepuParam.pager.nowPage=parseInt(res.page.pageIndex);
          This.kepuParam.pager.totalPage=parseInt(res.page.totalPages);
          This.kepuParam.load_more_loading=false;
          //确定没有更多时
          if(This.kepuParam.pager.nowPage+1>This.kepuParam.pager.totalPage){
            This.kepuParam.load_more_text="没有更多了";
            This.kepuParam.hasMore=false;
          }
        }
      },function(){
        This.kepuParam.load_more_loading=false;
