...
scroll_load ==>是否开启滚动加载 开启则滚动到页面底部就会加载，否则点击才会加载
 -->
<template>
  <div class="load-btn" :class="{'ending':isEnd}" @click='clickFn'>
     <div class="loading-text" :class="{'white':isWhite}" v-if="loading">
       <img width="24" height="24" src="./loading.gif">
       <span></span>正在加载内容
     </div>
      <div class="loading-text"  v-if="!loading">{{title}}</div>
  </div>
</template>
<script>
  export default {
