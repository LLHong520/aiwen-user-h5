...
const comtyIdValidMixin = {
  validate({ params, query, route, store}) {
    /**
     * 注： 校验页面是否有社区id，没有则返回404状态码
     * 注2： 由于没有单独主域名，nginx配置会把404状态码的转到其他地址（newshealth.sina)
     *   所以不需要携带comtyId的，需要在 exclude 添加地址
    * */
    let isExclude = excludeLinks.some( a => { return route.path.indexOf(a) != -1; });
    let hasComtyId = (query && query.comtyId) || store.state.comtyId;
    let result = hasComtyId || isExclude;

    return result;
  },
  async getComtyInfo({ redirect, store, query }) {
  ...
  async getArticleInfo({ route, redirect, store }, type) {
    //获取文章详情
    let res = {};
    let result = {};
    let params = {
    ...
    if(res.status == 200 && res.data){
      ...
      articleInfo.articleTags = articleInfo.tags && typeof articleInfo.tags == "array"
        ? articleInfo.tags.map( item => { return item; })
        : [];
