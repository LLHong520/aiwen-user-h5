...
const renderWithEndDate = function () {
  let date = new Date();
  let year = date.getFullYear();
  let month = date.getMonth() +1;
  let day = date.getDate();
  const yearData = range(START_YEAR, END_YEAR, false, UNIT_YEAR);
  const dateData = yearData.map((itemYear) => {
    let months = [];
    if (year == itemYear.slice(0, -1)) {
      months = cascadeMonthData.slice(0, month);
    }else{
      months = cascadeMonthData.slice(0, monthData.length);
    }
    const item = {
      value: itemYear,
      children: months
    ...
    item.children.forEach((itemMonth) => {
      let days = [];
      if (year == itemYear.slice(0, -1) && itemMonth.value.slice(0, -1) == month){
        days = getDays(itemYear.slice(0, -1), itemMonth.value.slice(0, -1)).slice(0, day);
      }else{
        days = getDays(itemYear.slice(0, -1), itemMonth.value.slice(0, -1));
      }
      itemMonth.children = days;
    });
    return item;
  });
